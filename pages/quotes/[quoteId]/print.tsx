...
export default function PrintableQuote({ quote }: any) {
  return (
	...
	<div className="text-right mb-4 space-y-1">
	  ...
	  <p><strong>Parts Total:</strong> ${quote.parts.reduce((sum, p) => sum + p.price, 0).toFixed(2)}</p>
	...
	{quote.signature && (
	  ...
	  />
	  <button type="button" onClick={() => sigCanvas.current?.clear()} className="mt-2 text-sm text-blue-600 underline">
		Clear Signature
	  </button>
	</div>
